...
static volatile uint8_t * const hw_max_limit_ddr[N_LIMIT_AXIS] = { &MAX_LIMIT_DDR(X_AXIS), &MAX_LIMIT_DDR(Y_AXIS), &MAX_LIMIT_DDR(Z_AXIS) };
static volatile uint8_t * const hw_max_limit_pins[N_LIMIT_AXIS] = { &MAX_LIMIT_PIN(X_AXIS), &MAX_LIMIT_PIN(Y_AXIS), &MAX_LIMIT_PIN(Z_AXIS) };
static volatile uint8_t * const hw_max_limit_port[N_LIMIT_AXIS] = { &MAX_LIMIT_PORT(X_AXIS), &MAX_LIMIT_PORT(Y_AXIS), &MAX_LIMIT_PORT(Z_AXIS) };
static const uint8_t hw_max_limit_mask[N_LIMIT_AXIS] = { MAX_LIMIT_MASK(X_AXIS), MAX_LIMIT_MASK(Y_AXIS), MAX_LIMIT_MASK(Z_AXIS) };

static volatile uint8_t * const hw_min_limit_ddr[N_LIMIT_AXIS] = { &MIN_LIMIT_DDR(X_AXIS), &MIN_LIMIT_DDR(Y_AXIS), &MIN_LIMIT_DDR(Z_AXIS) };
static volatile uint8_t * const hw_min_limit_pins[N_LIMIT_AXIS] = { &MIN_LIMIT_PIN(X_AXIS), &MIN_LIMIT_PIN(Y_AXIS), &MIN_LIMIT_PIN(Z_AXIS) };
static volatile uint8_t * const hw_min_limit_port[N_LIMIT_AXIS] = { &MIN_LIMIT_PORT(X_AXIS), &MIN_LIMIT_PORT(Y_AXIS), &MIN_LIMIT_PORT(Z_AXIS) };
static const uint8_t hw_min_limit_mask[N_LIMIT_AXIS] = { MIN_LIMIT_MASK(X_AXIS), MIN_LIMIT_MASK(Y_AXIS), MIN_LIMIT_MASK(Z_AXIS) };
...
void limits_init()
{
  ...
  for ( idx = 0; idx<N_LIMIT_AXIS; idx++ ) {
    ...
    bit_false( *hw_min_limit_ddr[idx], hw_min_limit_mask[idx] ); // configure as an input pin

    #ifdef DISABLE_LIMIT_PIN_PULL_UP
	  bit_false( *hw_max_limit_port[idx], hw_max_limit_mask[idx] );// Normal low operation. Requires external pull-down.
	  bit_false( *hw_min_limit_port[idx], hw_min_limit_mask[idx] );// Normal low operation. Requires external pull-down.
    #else
    ...
    if (bit_istrue(settings.flags,BITFLAG_HARD_LIMIT_ENABLE)) {
      ...
      WDTCSR = bit(WDP0); // Set time-out at ~32msec.
    #endif
...
// number in bit position, i.e. Z_AXIS is bit(2) or bit 2, and Y_AXIS is bit(1) or bit 1.
uint8_t limits_get_state()
{
  ...
  uint8_t idx;
  for (idx=0; idx<N_LIMIT_AXIS; idx++) {
    pin = bit_istrue( *hw_max_limit_pins[idx], hw_max_limit_mask[idx] );
    ...
    #endif
    if (pin) limit_state |= bit(idx);
    pin = bit_istrue( *hw_min_limit_pins[idx], hw_min_limit_mask[idx] );
    if (bit_isfalse(settings.flags,BITFLAG_INVERT_LIMIT_PINS)) { pin = !pin; }
...
void limits_go_home(uint8_t cycle_mask)
{
  ...
  uint8_t step_pin[N_AXIS];	// why is it an array ? for COREXY support ?
  float target[N_AXIS];	// not N_LIMIT_AXIS !
  float max_travel = 0.0;
  uint8_t idx;
  for (idx=0; idx<N_LIMIT_AXIS; idx++) {
    // Initialize step pin mask
